...
const int ncc_area = (2 * ncc_window_size + 1) * (2 * ncc_window_size + 1); // NCC窗口面积，窗口中像素个数
...
inline double getBilinearInterpolatedValue(const Mat &img, const Vector2d &pt)
{
    ...
    // d是指向数据块中的像素位置pt（浮点数）周围的四个像素中的左上角像素的指针
...
int main(int argc, char **argv)
{
    ...
    // 从数据集读取数据
    vector<string> color_image_files; // 每张图像的文件存储位置
    ...
    Vector2d &pt_curr)
{
    ...
    // 由于深度数据符合高斯分布，因此根据3σ原则，认为真实深度落在μ±3σ区间
    ...
    if (half_length > 100)
        half_length = 100; // 我们不希望搜索太多东西，100是指像素个数
    ...
    for (double l = -half_length; l <= half_length; l += 0.7) // l+=sqrt(2)/2，搜索步长
    {
    ...
    const Vector2d &pt_ref, const Vector2d &pt_curr)
{
    ...
    // ncc_window_size表征相似度计算窗口大小，即用来匹配的数据块大小
    ...
    Mat &depth_cov)
{
    ...
    double depth_estimation = d_esti.norm(); // 深度值，这里指的是空间点距相机坐标系原点的距离
