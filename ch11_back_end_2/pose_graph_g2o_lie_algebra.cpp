...
{
    Matrix6d J;
    J.block(0,0,3,3) = SO3::hat(e.so3().log());   // 旋转矩阵取对数得到李代数，再取反对称矩阵
    // 这里的/rou不应该是李代数里面的吗，与平移向量差着一个系数吧？？？？？？？？？？？？？？？？？？？？？？？？？？
...
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW   // 类中使用固定长度eigen数组，需要添加此句保证数据对齐
    bool read ( istream& is )
    {
        ...
        for ( int i=0; i<7; i++ )
            is>>data[i];
        setEstimate ( SE3 (
                Eigen::Quaterniond ( data[6],data[3], data[4], data[5] ),   // w为实部，x,y,z为虚部
    ...
    bool write ( ostream& os ) const
    {
        ...
        os<<_estimate.translation().transpose()<<" ";   // 先输出平移，再输出旋转
    ...
    {
        _estimate = Sophus::SE3();   // 初始化，无平移无旋转
    }
    // 左乘更新
...
public:
    ...
    bool read ( istream& is )
    {
        ...
        q.normalize();   // 对输入四元数进行归一化保证旋转矩阵正交
        setMeasurement (
    ...
    bool write ( ostream& os ) const
    {
        ...
        os<<v1->id()<<" "<<v2->id()<<" ";
        SE3 m = _measurement;   // 两个位姿之间的位姿变换矩阵，即相对运动测量
    ...
    virtual void computeError()
    {
        ...
        Sophus::SE3 v2 = (static_cast<VertexSE3LieAlgebra*> (_vertices[1]))->estimate();   // 变换矩阵
        _error = (_measurement.inverse()*v1.inverse()*v2).log();   // 位姿由v1变换到v2，误差为李代数表示
    }
    ...
    virtual void linearizeOplus()
    {
        ...
        // _jacobianOplusXi = - v2.inverse().Adj();
        // _jacobianOplusXj = v2.inverse().Adj();
    }
};
